...
如果启用自动 vault 处理，新的 setup 默认使用 `WIKI_AGENT_AUTH_MODE=codex-login`、当前用户 home 目录下的专用 Codex home 和 `WIKI_AGENT_MODEL=gpt-5.5`。启用前先执行 `CODEX_HOME="$HOME/.codex-tiangong-wiki" codex login` 完成专用 Codex 登录。
